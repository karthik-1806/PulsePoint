...
def generate_mock_venue_snapshot() -> VenueSnapshot:
    ...
    gates = [
        Gate(
            id=f"GATE_{i}",
            name=f"Gate {i}",
            status=random.choice(list(GateStatus)),
    ...
    zones = [
        Zone(
            id=f"ZONE_{i:02d}",
            name=f"Concourse Zone {i}",
            density_percent=random.uniform(10.0, 80.0),
        )
        for i in range(1, 13)
    ]
    ...
    for i in range(1, 21):
        ...
        sensors.append(
            Sensor(
                id=f"SENS_{i:03d}",
                zone_id=zone_id,
                density_percent=random.uniform(5.0, 95.0),
            )
        )
    ...
    transit = [
        TransitLine(
            id="TL_LR", type="light_rail", status="On Time", next_arrival_minutes=5
        ),
        TransitLine(id="TL_BUS", type="bus", status="Delayed", next_arrival_minutes=12),
        TransitLine(
            id="TL_FRY", type="ferry", status="On Time", next_arrival_minutes=45
        ),
    ]

    # 5. Weather
    weather = WeatherSnapshot(
        temperature_f=68.5, condition="Partly Cloudy", wind_speed_mph=5.2
    )
