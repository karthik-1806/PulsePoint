...
def generate_fallback_briefing(venue_data: dict) -> dict:
    ...
    if gates:
        ...
        items.append(
            {
                "type": "footfall",
                "message": f"Busiest gate is {busiest_gate[0]} at {busiest_gate[1].get('density', 0)}% density.",
            }
        )
    ...
    if weather:
        items.append(
            {
                "type": "weather",
                "message": f"{weather.get('condition', 'Clear')}, {weather.get('temperature', 70)}°F.",
            }
        )
    ...
    if parking:
        ...
        items.append(
            {
                "type": "parking",
                "message": f"Parking {most_full[0]} is {most_full[1]}% full.",
            }
        )
    ...
    items.append(
        {
            "type": "staffing",
            "message": "Staffing levels are nominal. Check roster for details.",
        }
    )
    ...
    if incidents:
        items.append(
            {
                "type": "incident",
                "message": f"There are {len(incidents)} active incidents.",
            }
        )
    else:
...
def generate_daily_briefing(venue_data_json: str) -> dict:
    ...
    if not os.getenv("GEMINI_API_KEY"):
        logger.error(
            "GEMINI_API_KEY not found. Falling back to deterministic briefing."
        )
    ...
    try:
        ...
        try:
            return call_gemini(
                "CRITICAL: Your previous response failed schema validation. Please strictly follow the JSON schema."
            )
