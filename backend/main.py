...
def fan_agent_endpoint(req: FanRequest):
    snapshot_json = venue_state.snapshot.model_dump_json() if venue_state.snapshot else "{}"
...
def report_incident(
    ...
    if roster:
        volunteer_match = find_best_volunteer(req.location, req.sanitized_description, roster)
        
    return {
        "classification": classification,
        "assigned_volunteer": volunteer_match
    }
...
def get_daily_briefing(user: dict = Depends(require_role(["commander", "marshal"]))):
    ...
    now = time.time()
    if briefing_cache["data"] and (now - briefing_cache["timestamp"] < 900): # 15 minutes
...
def get_forecast(request: Request, user: dict = Depends(require_role(["marshal", "commander"]))):
