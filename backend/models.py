...
class Gate(BaseModel):
    ...
    status: GateStatus = Field(
        ..., description="Current operational status of the gate"
    )
...
class Zone(BaseModel):
    ...
    density_percent: float = Field(
        0.0, description="Current crowd density (0.0 to 100.0)"
    )
...
class Sensor(BaseModel):
    ...
    density_percent: float = Field(
        0.0, description="Crowd density reading (0.0 to 100.0)"
    )
...
class TransitLine(BaseModel):
    ...
    type: str = Field(
        ..., description="Type of transit (e.g., 'light_rail', 'bus', 'ferry')"
    )
    status: str = Field(
        ..., description="Current operational status (e.g., 'On Time', 'Delayed')"
    )
    next_arrival_minutes: Optional[int] = Field(
        None, description="Minutes until next arrival"
    )
...
class WeatherSnapshot(BaseModel):
    ...
    condition: str = Field(
        ..., description="Weather condition (e.g., 'Sunny', 'Raining')"
    )
...
class Volunteer(BaseModel):
    ...
    skills: List[str] = Field(
        default_factory=list, description="List of relevant skills"
    )
    assigned_zone_id: Optional[str] = Field(
        None, description="Zone they are currently assigned to"
    )


class VolunteerRoster(BaseModel):
    volunteers: List[Volunteer] = Field(
        default_factory=list, description="All volunteers on shift"
    )
...
class FixtureSchedule(BaseModel):
    ...
    match_status: str = Field(
        ...,
        description="Current status of the match (e.g., 'Scheduled', 'In Progress')",
    )
