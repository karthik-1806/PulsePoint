...
def bounded_random_walk(current_val: float, min_val: float = 0.0, max_val: float = 100.0, step_size: float = 2.0) -> float:
    """
    ...
    while True:
        if venue_state.snapshot:
            ...
            for gate in venue_state.snapshot.gates:
                gate.wait_time_minutes = int(bounded_random_walk(gate.wait_time_minutes, min_val=0, max_val=120, step_size=2))
