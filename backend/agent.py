...
def setup_gemini():
    ...
    if not api_key:
        logger.critical("CRITICAL: GEMINI_API_KEY is missing or empty. Application cannot start without it.")
        sys.exit(1)
    
    genai.configure(api_key=api_key)

def get_fan_agent_response(query: str, step_free: bool, language: str, venue_data: str) -> dict:
